...
function makeCalculate(gen) {
    return function (attacker, defender, move, field) {
        return calculateGen8(gen, attacker, defender, move, field);
    };
}
exports.makeCalculate = makeCalculate;
exports.calculateSS = makeCalculate(SS);
function calculateGen8(gen, attacker, defender, move, field) {
    ...
    util_1.checkIntrepidSword(attacker);
    util_1.checkIntrepidSword(defender);
    ...
    var attackStat = move.category === 'Special' ? 'spa' : move.name === 'Body Press' ? 'def' : 'atk';
    ...
    else if (attacker.hasAbility('Gorilla Tactics') &&
    ...
    if (defender.hasItem('Zinc Band') && move.category === 'Special') {
        dfMods.push(0x1800);
        description.defenderItem = defender.item;
    }
...
function getEVDescriptionText(pokemon, stat, nature) {
    return (pokemon.evs[stat] +
        (natures_1.NATURES[nature][0] === stat ? '+' : natures_1.NATURES[nature][1] === stat ? '-' : '') +
        ' ' +
        stats_1.displayStat(stat));
}
